    ...
    def testValue(self):
        ...
        for b in range(batch_size):
            for h in range(num_heads):
                ...
                similarity = np.exp(similarity * strengths_data[b, h])
...
# class AllocationTest(tf.test.TestCase):
#
#     def testUsage(self):
#         batch_size = 5
#         memory_size = 11
#         num_reads = 3
#         num_writes = 7
#         module = addr.Allocation(memory_size)
#
...
#
#         usage = module.get_usage(
#             write_weights=tf.constant(prev_write_weights),
#             free_gates=tf.constant(free_gate),
...
#
#         self.assertAllClose(weights[1][0], np.eye(memory_size)[4], atol=1e-3)
#         self.assertAllClose(weights[1][1], np.eye(memory_size)[3], atol=1e-3)
#
...
#         usage = np.random.rand(batch_size, memory_size)
#         module = addr.Allocation(memory_size)
#         allocation = module.get_allocation(tf.constant(usage))
#         with self.test_session():
...
#         usage = tf.constant(np.random.rand(batch_size, memory_size))
#         module = addr.Allocation(memory_size)
#         allocation = module.get_allocation(usage)
#         with self.test_session():
#             err = tf.test.compute_gradient_error(
#                 usage,
...
#
#         state = addr.TemporalLinkageState(
...
#                 if i == num_steps - 2:
#                     write_weights[0, 0, :] = np.eye(memory_size)[0]
#                     write_weights[0, 1, :] = np.eye(memory_size)[3]
#                 elif i == num_steps - 1:
#                     write_weights[0, 0, :] = np.eye(memory_size)[1]
#                     write_weights[0, 1, :] = np.eye(memory_size)[2]
#
#                 state = sess.run(
#                     calc_state,
#                     feed_dict={
...
#         self.assertAllEqual(state.link[0, 0, :, 0], np.eye(memory_size)[1])
#         self.assertAllEqual(state.link[0, 1, :, 3], np.eye(memory_size)[2])
#
#         # Now test calculation of forward and backward read weights
#         prev_read_weights = np.random.rand(batch_size, num_reads, memory_size)
#         prev_read_weights[0, 5, :] = np.eye(memory_size)[0] # read 5, posn 0
#         prev_read_weights[0, 6, :] = np.eye(memory_size)[2] # read 6, posn 2
...
#
#         precedence_weights = module.precedence_weights(
