  ...
  *   `addressing.FreenessAllocator` for keeping track of memory usage, where
  ...
  def _read_inputs(self, inputs):
    ...
    def _linear(first_dim, second_dim, name, activation=None):
        """Returns a linear transformation of `inputs`, followed by a reshape."""
        linear = linear_transform(name=name, inputs=inputs, outputs_size=first_dim * second_dim)
    ...
    free_gate = tf.sigmoid(
        linear_transform(inputs=inputs,
                         outputs_size=self._num_reads,
                         name='free_gate'))
    ...
    write_keys = _linear(name='write_keys',
                         first_dim=self._num_writes,
                         second_dim=self._word_size)
