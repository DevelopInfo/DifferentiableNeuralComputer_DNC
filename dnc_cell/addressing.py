...
def weighted_softmax(activations, strengths, strengths_op):
  ...
  return tf.nn.softmax(logits=sharp_activations, axis=-1)


class CosineWeights(tf.nn.rnn_cell.RNNCell):
...
class TemporalLinkage(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, write_weight, prev_state):
    return self._build(write_weight, prev_state)
